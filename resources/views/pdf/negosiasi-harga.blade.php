...
<body>
    ...
    <table style="width: 100%">
        <tr>
            ...
            <td></td>
            <td></td>
            <td style="text-align: right">{{ ucwords(Auth::user()->desa) }}, {{ Illuminate\Support\Carbon::parse($pemberitahuan->tgl_batas_akhir_penawaran)->isoFormat('D MMMM Y') }}</td>
        </tr>
        <tr>
            <td>Nomor</td>
            <td style="width: 6cm">: {{ $pemberitahuan->no_pbj }}/Pers/{{ Auth::user()->kode_desa }}/{{ Auth::user()->tahun_anggaran }}</td>
        ...
        <tr>
            ...
            <td>: -</td>
            <td>Yth : </td>
            <td>{{ $penyedia->nama_pemilik . ' ('. $penyedia->nama_penyedia . ')' }}</td>
        </tr>
        <tr>
            <td>Perihal</td>
            <td>: Undangan</td>
    ...
    <br>
    {{-- <p style="margin-left:2.6cm; text-align:justify">Menanggapi surat penawaran harga Saudara Nomor {{ session('no_penyedia_1') }} tanggal {{ session('tgl_penyedia_1')->isoFormat('D MMMM Y') }} hal : Penawaran harga, bersama ini kami sampaikan hal-hal sebagai berikut :</p> --}}
...
{{-- @include('pdf.negosiasi-harga.kwitansi')  --}}
{{-- @include('pdf.negosiasi-harga.invoice')  --}}
