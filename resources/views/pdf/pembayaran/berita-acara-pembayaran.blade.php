...
<div style="line-height: 1.5">
    <p style="text-align: justify">
        PIHAK PERTAMA berdasarkan Surat Perjanjian Nomor {{ $kegiatan->nomor }}/PERJ/{{Auth::user()->kode_desa}}/{{Auth::user()->tahun_anggaran }} atas pekerjaan {{ $kegiatan->kegiatan }} telah membayar untuk pekerjaan {{ $kegiatan->kegiatan }}. kepada PIHAK KEDUA sebesar Rp. {{ number_format($kegiatan->negosiasiHarga->total, 0, ',', '.') }},-  <i>( {{ ucwords(Terbilang::make($kegiatan->negosiasiHarga->total)) }} Rupiah. )</i> <br>
        PIHAK KEDUA berdasarkan Surat Perjanjian Nomor {{ $kegiatan->nomor }}/PERJ/{{Auth::user()->kode_desa}}/{{Auth::user()->tahun_anggaran }} atas pekerjaan {{ $kegiatan->kegiatan }} telah melaksanakan pekerjaan {{ $kegiatan->kegiatan }} sesuai permintaan PIHAK PERTAMA dan telah menerima pembayaran atas pekerjaan tersebut sebesar Rp. {{ number_format($kegiatan->negosiasiHarga->total, 0, ',', '.') }},-  <i>( {{ ucwords(Terbilang::make($kegiatan->negosiasiHarga->total)) }} Rupiah. )</i>
    </p>
    <p style="text-align: justify">
