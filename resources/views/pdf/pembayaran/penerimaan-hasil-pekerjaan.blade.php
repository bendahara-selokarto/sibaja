...
<body style="font-family: serif; line-height: 1.2; margin: 40px 40px 40px 100px;">
    ...
    <p style="text-align: justify">Pada hari ini {{$tgl->isoFormat('dddd')}} tanggal {{Terbilang::make($tgl->isoFormat('DD'))}} bulan {{$tgl->isoFormat('MMMM')}} tahun {{Terbilang::make(Auth::user()->tahun_anggaran)}} bertempat di Kantor Kepala Desa {{Auth::user()->desa}}, telah dilaksanakan pembayaran atas pekerjaan pengadaan material {{$kegiatan->kegiatan}} antara :</p>
    ...
    <p style="text-align: justify">PIHAK PERTAMA menyatakan bahwa telah menerima hasil pekerjaan berupa Material kegiatan {{$kegiatan->kegiatan }} dalam keadaan baik dari PIHAK KEDUA sesuai dengan Surat Perjanjian Nomor : {{$pemberitahuan->no_pbj}}/PERJ/{{Auth::user()->kode_desa}}/{{Auth::user()->tahun_anggaran}}  tertanggal {{tanggal_indo($tgl)}}.</p>

    <p style="text-align: justify">PIHAK KEDUA telah menyerahkan hasil pekerjaan berupa Material kegiatan {{$kegiatan->kegiatan }} dalam keadaan baik kepada PIHAK PERTAMA sesuai dengan Surat Perjanjian Nomor Nomor : {{$pemberitahuan->no_pbj}}/PERJ/{{Auth::user()->kode_desa}}/{{Auth::user()->tahun_anggaran}}  tertanggal {{tanggal_indo($tgl)}}.</p>
