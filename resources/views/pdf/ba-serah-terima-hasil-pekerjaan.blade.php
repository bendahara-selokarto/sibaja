...
<body style="margin-left: 3cm; margin-top: 1cm; margin-right: 2cm; margin-bottom: 2cm;">
    ...
    <p style="margin-top: 1rem; text-align: justify; font-size: 12pt;">
        Pada hari ini, {{ $hari ?? 'Senin' }}, tanggal {{ $tanggal ?? '01' }} bulan {{ $bulan ?? 'Januari' }} tahun {{ $tahun ?? '2023' }}, bertempat di {{ $tempat ?? 'Kantor' }}, telah dilaksanakan serah terima hasil pekerjaan antara:
        ...
        <table style="font-size: 12pt; border-collapse: collapse; width: 100%; margin-bottom: 1.5rem;">
            ...
            <td style="width: 50%; padding: 4px;">{{ $pihak_pertama['nama'] ?? '-' }}</td>
            </tr>
            <tr>
            <td style="padding: 4px;">Jabatan</td>
            <td style="padding: 4px;">{{ $pihak_pertama['jabatan'] ?? '-' }}</td>
            </tr>
            <tr>
            <td style="padding: 4px;">Alamat</td>
            <td style="padding: 4px;">{{ $pihak_pertama['instansi'] ?? '-' }}</td>
            </tr>
            <tr>
            ...
            <td style="padding: 4px;">{{ $pihak_kedua['nama'] ?? '-' }}</td>
            </tr>
            <tr>
            <td style="padding: 4px;">Jabatan</td>
            <td style="padding: 4px;">{{ $pihak_kedua['jabatan'] ?? '-' }}</td>
            </tr>
            <tr>
            <td style="padding: 4px;">Instansi</td>
            <td style="padding: 4px;">{{ $pihak_kedua['instansi'] ?? '-' }}</td>
            </tr>
            <tr>
        ...
        <table style="width: 100%; margin-top: 2rem; font-size: 12pt; text-align: center;">
            <tr>
                <td style="width: 50%;">
                    Kepala Desa Selokarto<br>Selaku<br>Pemegang Kekuasaan Pengelolaan<br>Keuangan Desa<br>
                    <u>{{ $pihak_kedua['nama'] ?? '-' }}</u><br>
                    {{ $pihak_kedua['jabatan'] ?? '-' }}
                </td>
                <td style="width: 50%;">
                    Pelaksana Kegiatan Anggaran<br><br><br><br>
                    <u>{{ $pihak_pertama['nama'] ?? '-' }}</u><br>
                    {{ $pihak_pertama['jabatan'] ?? '-' }}
                </td>
            </tr>
        </table>
