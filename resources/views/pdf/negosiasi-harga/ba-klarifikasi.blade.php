    ...
    <h4 style="margin: 0px; text-align:center">Nomor :  {{ $pemberitahuan->no_pbj }} /BA-KLNH/{{ Auth::user()->kode_desa }}/ {{ Auth::user()->tahun_anggaran }}</h4>
    <h4 style="text-align:center">Tanggal : {{ $negosiasiHarga->tgl_negosiasi->isoFormat('D MMMM Y') }}</h4>
    @php
    @endphp

    <p style="text-align:justify; text-indent:1cm;">
    Pada hari ini <strong>{{ $negosiasiHarga->tgl_negosiasi->isoFormat('dddd') }}</strong> 
    tanggal <strong>{{ Terbilang::make($negosiasiHarga->tgl_negosiasi->isoFormat('D')) }}</strong> 
    bulan <strong>{{ $negosiasiHarga->tgl_negosiasi->isoFormat('MMMM')}} </strong>
    ...
    <tr>
        ...
        <td style="">:</td>
        <td style="">{{ $penyedia->nama_penyedia }}<</td>
    </tr>
    <tr>
        <td></td>
        <td>Alamat</td>
    ...
    <tr>
        ...
        <td>:</td>
        <td>{{ number_format($penawaranHarga->harga_penawaran_1, 0, ',', '.') }} ( {{ Terbilang::make($penawaranHarga->harga_penawaran_1) }} rupiah )</td>
    </tr>
    <tr>
        <td></td>
        <td>Harga Negosiasi</td>
        <td>:</td>
        <td>{{ number_format($negosiasiHarga->harga_negosiasi, 0, ',', '.') }} ( {{ Terbilang::make($negosiasiHarga->harga_negosiasi) }} rupiah )</td>
    </tr>
    <tr>
...
<table style="width: 100%">
    <tr>
        <td style="text-align: center">Menyetujui,<br>{{ $penyedia->nama_penyedia }} <br><br><br><br><span>{{ $penyedia->nama_pemilik}}</span></td>
        <td style="text-align: center">Tim Pengelola Kegiatan<br> Ketua <br><br><br><br><span>{{ $kegiatan->ketua_tpk}}</span></td>
   </tr>
</table>
</div>
